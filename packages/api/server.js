...
app.get('/mediusa', async (req, res) => {
  fs.readFile('mediusaData.json', 'utf8', async (err, json) => {
    ...
    async.eachLimit(
      ...
      err => {
        ...
        fs.writeFile(
          'mediusaData2.json',
          JSON.stringify({ finalData, errors }),
          'utf8',
          () => {
            console.log('done')
          }
  ...
  // fs.readFile('mediusad.json', 'utf8', async (err, json) => {
...
app.get('/mediusatrans', (req, res) => {
  fs.readFile('mediusaData2.json', 'utf8', async (err, json) => {
    const data = JSON.parse(json)
    const newData = data.finalData.map(x => ({
      ...x,
...
app.get('/mediusahh', (req, res) => {
  fs.readFile('mediusaData3.json', 'utf8', async (err, json) => {
    const products = JSON.parse(json)
    const filteredProducts = products.filter(
      product => product.childrens.length > 0
    )
...
app.get('/medium', async (req, res) => {
  ...
  const baseUrl = 'https://medium.com/_/graphql'
  async function fetchPostsForTag(tag) {
    ...
    let cursor = 0
    try {
      while (posts.length < 2000 && cursor !== null) {
          ...
          body: JSON.stringify([
            {
              ...
              variables: {
                tagSlug: tag,
                mode: 'HOT',
                paging: { limit: 25, to: cursor.toString() }
              },
              query:
        ...
        const edges = result[0].data?.tagFeed?.items
        cursor = result[0].data?.tagFeed?.pagingInfo?.next?.to || null
        if (edges?.length)
          for (const edge of edges) {
            ...
            posts.push(edge)
            console.log(
              `Fetched post ${edge.post.id} total posts: ${posts.length}`
            )
          }
      }
  ...
  async function savePostsToFile(posts) {
    ...
    fs.writeFileSync('post.json', json)
  }
...
app.get('/medium/post', async (req, res) => {
  fs.readFile('postids.json', 'utf8', (err, data) => {
    if (err) {
      console.log(err)
    }
    ...
    const fetchAllData = async () => {
      const finalData = []
      const categories = Object.keys(postIds)
      for (let i = 0; i < categories.length; i++) {
        const category = categories[i]
        const ids = postIds[category]
        for (let j = 0; j < ids.length; j++) {
          const id = ids[j]
          const result = await fetchData(id)
          if (result) {
            finalData.push(result)
            console.log(`total data fetched: ${finalData.length}`)
          }
        }
      }
      return finalData
    ...
    fetchAllData()
      .then(finalData =>
        fs.writeFileSync('data.json', JSON.stringify(finalData), err => {
          if (err) {
            console.error(err)
            return
          }
...
app.get('/medium/users', async (req, res) => {
  // Define batch size for processing
  const BATCH_SIZE = 500
  console.log('Processing posts in batches of', BATCH_SIZE)

  const rawData = fs.readFileSync('data.json')
  const AllPostsData = JSON.parse(rawData)
  const errrors = []
  // Define function to extract creator data from post object
  try {
    function extractCreatorData(post) {
      const { creator } = post.data.postResult
      return {
        image: `https://miro.medium.com/v2/${creator?.imageId}`,
        email: `${creator?.id}@sanchaar.com`,
    ...
    // Create a set to store unique creators
    const uniqueCreators = new Set()

    // Process data in batches
    for (let i = 0; i < AllPostsData.length; i += BATCH_SIZE) {
      const batch = AllPostsData.slice(i, i + BATCH_SIZE)
      console.log('Processing batch', i, 'to', i + BATCH_SIZE)
      for (const item of batch) {
        console.log('Processing post', item[0].data.postResult?.id)
        const creatorData = extractCreatorData(item[0])
        uniqueCreators.add(JSON.stringify(creatorData))
      }
    }

    // Convert unique creators set back to an array of objects
    const uniqueCreatorArray = Array.from(uniqueCreators).map(JSON.parse)
    res.json(uniqueCreatorArray)
  } catch (error) {
    errrors.push(error)
